...
USE Generic_List, ONLY : LI_Init_List,LI_Add_To_Head,LI_Add_To_Tail,&
     LI_Get_Head,LI_Get_Tail,LI_Remove_Head,LI_Remove_Tail,&
     LI_Get_Next,LI_Associated,LI_Get_Len
...
! Auxilliary data type required for the transfer function
TYPE box_ptr
  TYPE(box_node), POINTER :: node
END TYPE box_ptr

TYPE(List_Type)      :: box_list, sublist
TYPE(Link_Ptr_Type)  :: link, sublink
TYPE(box_ptr)        :: box1, box2
...
DO I=1,N
   ALLOCATE(box1%node)
   ALLOCATE(box1%node%mybox)
   call setup_base_box(box1%node%mybox, 10+i, 3)
   Link = TRANSFER(box1,link)
!!$   CALL LI_Add_To_Head(link, box_list)
   CALL LI_Add_To_Tail(link, box_list)
ENDDO

! Cycle through list
Link = LI_Get_Head(box_list)
DO WHILE(LI_Associated(Link))
   box1 = TRANSFER(Link,box1)
   WRITE(*,*) box1%node%mybox%npoints
   Link = LI_Get_Next(Link)
ENDDO

! Find list length
WRITE(6,*)
WRITE(6,*)'Length = ',LI_Get_Len(box_list)
WRITE(6,*)
